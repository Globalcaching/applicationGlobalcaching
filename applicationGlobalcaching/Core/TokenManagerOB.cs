...
{
    public class TokenManagerOB
    {
        private static string WcfTokenManagerTag = "WcfTokenManagerOB";
        private static string WcfAccessTokenTag = "WcfAccessTokenOB";
        private static string WcfAccessTokenSecretTag = "WcfAccessTokenSecretOB";
        ...
        public static void RequestAuthorization(System.Web.UI.Control ctrl)
        {
            ...
            UriBuilder callback = new UriBuilder(ctrl.Page.Request.Url.AbsoluteUri.Replace("TokenRequestOB","TokenProcessOB")); // the opencaching oauth handler will call back to this page
        ...
        private static WebConsumer CreateConsumer(System.Web.UI.Control ctrl)
        {
            ...
            oauthRequestTokenEndpoint = new MessageReceivingEndpoint(new Uri(GetConfigSetting("ocnl_oauth_request")), HttpDeliveryMethods.PostRequest);
            oauthUserAuthorizationEndpoint = new MessageReceivingEndpoint(new Uri(GetConfigSetting("ocnl_oauth_authorize")), HttpDeliveryMethods.GetRequest);
            oauthAccessTokenEndpoint = new MessageReceivingEndpoint(new Uri(GetConfigSetting("ocnl_oauth_gettoken")), HttpDeliveryMethods.PostRequest);
            ...
            {
                tokenManager = new InMemoryTokenManager(GetConfigSetting("ocnl_consumerkey"), GetConfigSetting("ocnl_consumersecret"));
